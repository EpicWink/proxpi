...
class Subscriber:
    ...
    download_path: str
    read_position: int = 0
    # we shall use other methods to let pub to replace first, then sub read
    notify_queue: queue.Queue[int | RequestException] = queue.Queue()
...
class DownloadStatus:
    ...
    @staticmethod
    def _notify(subscriber: Subscriber, data: int | RequestException):
        """
        NOTE: ALWAYS acquire _notify_lock before calling
        """
        try:
            subscriber.notify_queue.put_nowait(data)
        except queue.Full:
            logger.debug(f"notify fail on {subscriber}")
            pass

    def _notify_all(self, data: int | RequestException):
        with self._notify_lock:
            for subscriber in self._subscribers.values():
                self._notify(subscriber, data)
    ...
    def add_subscriber(self) -> Subscriber:
        ...
        subscriber = Subscriber(subscriber_id, self._temp_download_path)
        self._subscribers[subscriber_id] = subscriber
        logger.debug(
            f"{self._temp_download_path}: add, now {len(self._subscribers)} subs"
        )

        if self._pub_eof:  # send again to wakeup
            self._notify(subscriber, self._error)
    ...
    def remove_subscriber(self, subscriber: Subscriber):
        with self._notify_lock:
            ...
            logger.debug(
                f"{self._temp_download_path}: remove, now {len(self._subscribers)} subs"
            )
    ...
    def broadcast_eof(self, error: RequestException | int = -1):
        """Broadcast EOF (with exception) to all subscribers known."""
...
class _FileCache:
    ...
    def __init__(
        ...
        self._status_init_lock = threading.Lock()
    ...
    def _download_file(self, url: str, path: str, status: DownloadStatus):
        ...
        try:
            ...
            def cleanup():
                ...
                # Remove entry
                with self._status_init_lock:
                    key = self._get_key(url)
                    if key in self._files:
                        del self._files[key]

            status.set_cleanup_callback(cleanup)
            status.broadcast_eof(e)
            return
    ...
    def _start_downloading(self, url: str):
        ...
        self._status_init_lock.release()
    ...
    def get(self, url: str) -> str:
        ...
        self._status_init_lock.acquire()
